...
| `ci-ok` | **Aggregates every row above except the CodeQL contexts.** Issue #75 P2.4: `main`'s ruleset named all 11 workflow jobs individually, so each rename or matrix change had to be mirrored into the ruleset and into 12 committed files — #71's `test` → `test (3.9)`…`test (3.12)` rename took three commits and still missed `.github/pull_request_template.md`. `ci-ok` `needs:` the ten gating jobs (the eight above plus `changes` and `repo-invariants`, both added by specs/016-ci-path-based-selection — `changes` is the path-selection filter producer, `repo-invariants` reruns two repository-wide static tests that path selection cannot safely skip) and asserts each result explicitly (`if: always()` does not fail a job implicitly, so an unasserted aggregate reports green while `lint` is red). `performance`, `quality-summary`, `deploy-docs` and `sync-agent-integrations` are deliberately excluded, enforced by `tests/static/test_ci_ok_aggregate_check.py` | push, pull_request | Any of the ten gating jobs failing or being cancelled, **plus** an unexpected `skipped`. Being **skipped** does not block for exactly two cases: (1) one of the seven path-filtered jobs (`lint`/`complexity`/`typecheck`/`security`/`test`/`build`/`docs`, conditional on which paths a pull request touched — see that spec's contracts/path-selection-contract.md); (2) `changes` on a `workflow_dispatch` run, which every filtered job's `if:` already bypasses its output for. Every other `skipped` blocks exactly like a failure: `changes` skipping on `push`/`pull_request` (outside case 2), `dependency-scan`, and `repo-invariants` (which has no category-based `if:` at all and is never legitimately skipped) are never path-selected, so an unexpected skip of any of them is treated as a broken run, not a benign one. |

¹ **Since `specs/016-ci-path-based-selection`**, this job also skips (reports `skipped`, not
`success`) on a `push`/`pull_request` run whose changed paths fall outside the categories that
job depends on — subject to Constitution Principle IX's path-based job selection exception.
`dependency-scan` and CodeQL are not marked: FR-006 exempts `dependency-scan` from path
selection entirely, and CodeQL is GitHub-managed, outside `ci.yml`. See
`specs/016-ci-path-based-selection/contracts/path-selection-contract.md` for the full mapping.
