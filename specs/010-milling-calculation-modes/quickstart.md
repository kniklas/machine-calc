...
SC-001 for the two new modes; research.md #5): standard mode is 14
prompts / 13 typed values (the new mode prompt is itself non-dismissible
per FR-001a, so only the optional power prompt remains dismissible);
power-constrained mode is 14 prompts / 14 typed values (the mode prompt
adds one, and the now-required available-power prompt converts from
optional to typed, so every prompt in this mode requires a typed value);
fixed-RPM mode is 15 prompts / 14 typed values (the mode prompt plus the
required target-RPM prompt add two, offset by the optional advisory
available-power prompt remaining a single-Enter default — the only
dismissible prompt in this mode). `tests/integration/test_cli_prompt_budget.py`
