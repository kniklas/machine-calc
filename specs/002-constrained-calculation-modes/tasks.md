...
- [ ] T009 [P] Unit tests for `validate_target_rpm()` and `validate_mode_arguments()` (zero/negative/non-numeric/`NaN`/`Infinity` target RPM, all asserting `INVALID_TARGET_RPM`; both power-constraint and target RPM supplied; power-constrained mode without available_power; target_rpm supplied while mode is not FIXED_RPM) in `tests/unit/shared/test_validation_modes.py` (depends on T005)
...
- [ ] T010 [P] [US1] Contract test for the power-constrained success response shape (adjusted spindle speed, unchanged torque, `power_required` within `math.isclose(rel_tol=1e-9)` of `available_power`, `mode=POWER_CONSTRAINED`) per contracts/library-api-delta.md in `tests/contract/test_library_api_power_constrained.py`
