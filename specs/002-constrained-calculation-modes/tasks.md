...
- [ ] T015 [US1] When `power-constrained` mode is selected, replace the existing optional advisory available-power prompt with a required available-power prompt in `src/machine_calc/cli.py`; a blank or non-numeric entry MUST be re-prompted as a validation failure (never treated as `MODE_CONFLICT`, per spec.md Clarifications 2026-07-11), using the validation message from T005/T007 (FR-002; depends on T007, T014)
