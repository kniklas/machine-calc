...
| `any_invalid_memory_measurement` | `bool` | `any(not report.memory_measurement_valid for report in run)` (issue #23) — whether *any* case's memory reading was missing, zero, or negative (an unusable/broken measurement), as distinct from `cpu_pin_enforced_overall`/`memory_ceiling_enforced_overall` which describe *enforcement* (e.g. best-effort mode on macOS, which MAY still legitimately report a valid, passing reading with the ceiling merely unenforced per FR-009/FR-010 — note this does NOT extend to Windows, where the `resource` module itself is unavailable, so `measured_memory_bytes` can never be read at all and every case's measurement is always invalid; see the platform-capability contract). A `True` value always forces `status_label = "fail"` per the precedence rule above, regardless of `overall_pass` or the enforcement flags. |
