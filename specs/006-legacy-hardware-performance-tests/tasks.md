...
- [X] T001 Create the `tests/performance/` package directory with an empty `tests/performance/__init__.py`, per plan.md's Project Structure
- [X] T002 [P] Register the `performance` marker in `pyproject.toml`'s `[tool.pytest.ini_options]` section (add a `markers = ["performance: opt-in legacy-hardware time/memory budget checks (see tests/performance/)"]` entry) — leave `testpaths` and `addopts` on lines 53-54 unchanged so the default/blocking command is unaffected (FR-006, research.md #1)
...
- [X] T024 [P] Add a brief mention of the opt-in performance suite (its command and a link to `specs/006-legacy-hardware-performance-tests/quickstart.md`) to README.md's existing "Run the tests" section, making clear it is separate from and does not affect the `pytest` command already documented there
- [X] T025 Run the full `quickstart.md` validation end-to-end (all five scenarios in order) as a final confirmation that FR-001 through FR-012 and SC-001 through SC-006 are all satisfied together
...
- Avoid: modifying `tests/unit`, `tests/integration`, `tests/contract`, `tests/static`, or any `src/` file; adding a new third-party dependency

---

## Phase 7: Convergence

- [X] T026 Correct the per-platform memory-ceiling enforcement claims in spec.md's Assumptions bullet, research.md's #3 Decision/Rationale, plan.md's Target Platform section, contracts/performance-suite-contract.md's Platform-capability contract table, and quickstart.md's Scenario 3 — these currently state that macOS reliably/guaranteed enforces the memory ceiling (`memory_ceiling_enforced=True`) via `resource.setrlimit(RLIMIT_AS, ...)`, but verified local execution on a macOS dev machine shows all four cases report `memory_ceiling_enforced=False`, because `setrlimit` raises `ValueError`/`OSError` when the interpreter's own address space already exceeds the 128 MB ceiling before the call — `harness.py`'s `enforce_memory_ceiling()` already handles this correctly per FR-009 (catches `ValueError`/`OSError`, degrades gracefully, never crashes), so no code change is needed, only updating these docs to describe macOS memory-ceiling enforcement as best-effort/commonly-degraded rather than guaranteed, per spec.md Assumptions / research.md #3 / plan.md Target Platform / contracts/performance-suite-contract.md (contradicts)
- [X] T027 Perform the deferred real-pull-request validation for T019 and quickstart.md Scenario 5 now that this feature's branch is ready to open a PR: open the pull request, confirm the `performance` job appears in its checks list and runs on `ubuntu-latest` executing Scenario 1's command, confirm the PR's required/blocking status checks (`lint`, `complexity`, `typecheck`, `security`, `dependency-scan`, `test`, `build`, `docs`) and overall mergeability are unaffected by the `performance` job's outcome, and confirm `performance` does not appear as a row in the `quality-summary` PR comment, per US2/T019, quickstart Scenario 5 (partial)

---

## Phase 8: Quality Check Summary Integration (FR-013)

- [X] T028 In `.github/workflows/ci.yml`'s `performance` job, add a step (with `id:`, after the existing `continue-on-error: true` test step) that computes the run's Suite Run Summary (data-model.md) from the suite's per-case results and writes two `$GITHUB_OUTPUT` variables: (1) `status_label` — one of `pass`, `fail`, `⚠️ degraded`, `skipped`, `cancelled`, where `⚠️ degraded` is set whenever *either* the run-level single-core enforcement flag or the run-level memory-ceiling enforcement flag was `False` for that run (a simple boolean OR of "was inactive", independent of the measured pass/fail outcome — FR-013 Clarifications #3/#5), and (2) `metric` — the achieved-key-metric string (T028 continued below) — do NOT rely on `needs.performance.result`, since `continue-on-error: true` on the test step causes GitHub Actions to always report the job's own `result` as `success` regardless of the suite's actual outcome (contracts/ci-performance-job-contract.md Non-goals)
- [X] T028b In that same step, build the `metric` output as a single string with the worst-case (highest) measured wall-clock time and worst-case (highest) measured peak memory across *all* Performance Reports in the run plus their budgets — format `f"{worst_time:.2f}s / {worst_memory_mb}MB (budgets: {time_budget}s/{memory_budget_mb}MB)"`, e.g. `0.42s / 58MB (budgets: 1.0s/128MB)` — never a per-case breakdown (FR-013 Clarifications #1). Show this real measured string whenever at least one per-case measurement was actually produced, including on a genuine budget failure and on a degraded-but-measured run — only fall back to the standard `—` "no metric available" placeholder (specs/004-pr-quality-check-summary FR-005) when the job was skipped, cancelled, or degraded before producing any measurement at all (FR-013 Clarifications #2/#4)
- [X] T029 Declare the new step's `status_label` and `metric` outputs under the `performance` job's `outputs:` block in `.github/workflows/ci.yml` (mirroring the existing `complexity.outputs.mi_summary` and `test.outputs.coverage_pct` pattern)
- [X] T030 Add `performance` to `quality-summary`'s `needs:` list in `.github/workflows/ci.yml`, and add a `PERFORMANCE_STATUS`/`PERFORMANCE_METRIC` env pair (sourced from `needs.performance.outputs.status_label`/`needs.performance.outputs.metric` per T029) to the `Build summary` step
- [X] T031 Add a `| performance | ... |` row to the Markdown table built in `quality-summary`'s `Build summary` step, rendering `PERFORMANCE_STATUS` with the same status-label rendering used for the other rows' `pass`/`fail`/`skipped`/`cancelled` labels but extended to also render the literal `⚠️ degraded` label when `PERFORMANCE_STATUS` is `⚠️ degraded`, and rendering `PERFORMANCE_METRIC` verbatim (it is already either the formatted metric string or the `—` placeholder per T028b, so no additional `metric_or_placeholder` substitution is needed for this row)
- [X] T032 Ensure `performance`'s status is explicitly excluded from the `results="..."` string used to compute overall status in `quality-summary`'s `Build summary` step for *every* value it can take — including `⚠️ degraded`, not just `fail`/`skipped`/`cancelled` — so a failing or degraded performance result never flips the overall status to FAIL or CANCELLED (spec.md FR-013; specs/004-pr-quality-check-summary FR-011)
- [X] T033 Validate end-to-end on a real PR: confirm the `performance` row appears in the `quality-summary` comment with the exact `"<time>s / <mem>MB (budgets: 1.0s/128MB)"` metric format, confirm a deliberately-failing performance run still shows the real measured values (not the `—` placeholder) in that row, confirm a deliberately-degraded (single-core or memory-ceiling enforcement disabled) run shows the `⚠️ degraded` label, confirm a skipped/cancelled run shows the `—` placeholder, confirm all of the above still leave the comment's overall status as PASS (when all other checks pass), and confirm the PR remains mergeable
