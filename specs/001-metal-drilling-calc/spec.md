...
- **FR-019**: All user-facing text presented by the interactive text interface (REPL prompts, labels, validation/error messages, feasibility warnings) MUST be sourced from a message catalog (keyed by a stable, language-independent identifier) rather than hard-coded inline strings. The library API MUST accept an optional `locale` parameter (defaulting to English) and MUST return `ErrorInfo.message` and any warning text localized per that parameter, using the same catalog as the interactive text interface — the library is not limited to English-only output. The module MUST bundle an English catalog as the default and fallback locale; if a requested locale or a specific message key is missing from a non-English catalog, the module MUST fall back to the English entry for that key rather than fail or show a blank message. This requirement does not apply to internal application/diagnostic logging, which MUST always be written in English regardless of the active user-facing locale.
