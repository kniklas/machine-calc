...
- **FR-019**: All user-facing text presented by the interactive text interface (REPL prompts, labels, validation/error messages, feasibility warnings) MUST be sourced from a message catalog: one pure-Python module per locale (e.g. `locales/en.py`, exposing a `dict[str, str]` of message ID → string), keyed by a stable, language-independent identifier, rather than hard-coded inline strings or a JSON/YAML/gettext-based mechanism. Catalog strings requiring dynamic values (e.g. a material or tool name) MUST use `str.format()`-style named placeholders (e.g. `"Unknown workpiece material: {material!r}."`), populated by the caller at lookup time. The interactive text interface MUST select its locale solely from the `MACHINE_CALC_LOCALE` environment variable at startup (falling back to English if unset or unrecognized), without adding a new interactive prompt step and without any OS/system locale (e.g. `LANG`/`LC_ALL`) auto-detection. The library API MUST accept an optional `locale` parameter (defaulting to English) and MUST return `ErrorInfo.message` and any warning text localized per that parameter, using the same catalog as the interactive text interface — the library is not limited to English-only output. The module MUST bundle an English catalog module as the default and fallback locale; if a requested locale or a specific message key is missing from a non-English catalog module, the module MUST fall back to the English entry for that key rather than fail or show a blank message. This requirement does not apply to internal application/diagnostic logging, which MUST always be written in English regardless of the active user-facing locale.
...
- **SC-007**: A new supported language can be added by providing a new message catalog file/config only, with zero changes to calculation or validation logic, and the application still functions correctly (falling back to English) if that catalog is incomplete.
...
- This feature ships only the English message catalog (FR-019); it establishes the message-catalog infrastructure so additional languages can be added later by contributing a new locale module, without requiring a translated catalog to exist as part of this feature.
