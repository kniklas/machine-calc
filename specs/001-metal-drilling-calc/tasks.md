...
- [x] T037 Create the GitHub Actions CI workflow (`.github/workflows/ci.yml`): `ruff check` + `black --check` lint, `pytest --cov` with the 90% threshold, package build check (`python -m build`), and Sphinx docs build on every push/PR; all MUST pass before merge (research.md #8; Constitution Principle II, Additional Constraints). **Superseded/fulfilled by `specs/003-ci-quality-security-gates`**: `.github/workflows/ci.yml` now exists with `lint`, `test`, `build`, `docs` jobs exactly as described here, plus `complexity`, `typecheck`, `security`, `dependency-scan`, and CodeQL added by that feature — see `specs/003-ci-quality-security-gates/contracts/ci-checks-contract.md` for the authoritative, current required-checks list rather than duplicating it here.
- [x] T038 Add the GitHub Pages docs-publish step/job to `.github/workflows/ci.yml` (or a dedicated `docs.yml`), publishing the Sphinx build output on successful builds of `main` (Constitution VII; depends on T037, T035, T036). Done: GitHub Pages enabled (Actions build type) via `gh api -X POST repos/kniklas/machine-calc/pages -f build_type=workflow`; `.github/workflows/ci.yml`'s `docs` job now uploads the Sphinx HTML build via `actions/upload-pages-artifact@v3`, and a new `deploy-docs` job (gated to `push` events on `main` only, least-privilege `pages: write`/`id-token: write` permissions scoped to that job) deploys it via `actions/deploy-pages@v4`. Live at https://kniklas.github.io/machine-calc/; linked from README.md.
