...
- [ ] T044 [CRITICAL] Add a GitHub Actions CI workflow (`.github/workflows/ci.yml`) that runs on every push/pull request: `ruff check` + `black --check`, `pytest --cov=machine_calc --cov-report=term-missing` with the 90% coverage gate, a package build check (`python -m build`), and a Sphinx documentation build; all MUST pass before merge (Constitution Principle II, Additional Constraints) (missing)
- [ ] T045 [CRITICAL] Add a GitHub Pages docs-publish job to `.github/workflows/ci.yml` (or a dedicated `docs.yml`) that automatically publishes the Sphinx build output on every successful build of the `main` branch (Constitution Principle VII, Additional Constraints; depends on T044, T048) (missing)
- [ ] T046 [CRITICAL] Add a GitHub Actions release workflow (`.github/workflows/release.yml`) that builds and publishes the package to PyPI on every merge to `main` (Constitution Additional Constraints) (missing)
- [ ] T047 [CRITICAL] Update `README.md` to actually report the current unit test coverage level (percentage, from the most recent `pytest --cov` run), replacing the placeholder note that coverage reporting is merely "tracked" for later (Constitution Principle VII) (partial)
- [ ] T048 [HIGH] Write the Sphinx end-user guide (`docs/source/user-guide.rst`) and developer guide (`docs/source/developer-guide.rst`) — installation, CLI walkthrough, example library snippet, architecture overview, `operations/` extensibility pattern (Constitution Principle VI), and autodoc-generated API reference for `machine_calc` (Constitution Principle VII; plan.md Phase 5 T035/T036) (missing)
- [ ] T049 [HIGH] Implement a material/drilling-tool combination-support check in `operations/drilling/__init__.py` so that an unsupported pairing is rejected with the documented `UNSUPPORTED_COMBINATION` error code before any calculation, or — if the current multiplicative model genuinely makes every material×tool pairing supported by design — update spec.md FR-010, data-model.md, and contracts/library-api.md to remove the now-unreachable `UNSUPPORTED_COMBINATION` code and its quickstart.md reference accordingly (FR-010) (partial)
- [ ] T050 [HIGH] Add dedicated library-API contract tests per contracts/library-api.md: success-response shape (`tests/contract/test_library_api_success.py`), every documented error code including `UNSUPPORTED_COMBINATION` never raising (`tests/contract/test_library_api_errors.py`, depends on T049), and feasibility-warning behavior (`tests/contract/test_library_api_feasibility.py`) (FR-012, FR-013, FR-015; tasks T027-T029) (missing)
- [ ] T050a [HIGH] Add a contract test asserting `calculate()`'s docstring documents the exact unit for every `CalculationResult` field under both `UnitSystem.METRIC` and `UnitSystem.IMPERIAL` (`tests/contract/test_library_api_unit_labels.py`) (FR-013; task T027a) (missing)
- [ ] T051 [HIGH] Add an integration test proving identical `CalculationResult` values from direct `calculate()` calls and from driving the CLI with the same inputs (`tests/integration/test_identical_results.py`) (FR-016; task T030) (missing)
- [ ] T052 [MEDIUM] Execute all 8 quickstart.md scenarios (manually or via a validation script) and record actual vs. expected outcomes; explicitly time a full end-to-end CLI session and confirm it completes in under 30 seconds (SC-001; task T041) (missing)
- [ ] T053 [LOW] Measure and record calculation execution time and peak memory (RSS) usage against the 0.5-1.0s / ~64-128MB legacy-hardware targets in Constitution Principle V, documenting methodology and results in research.md or a new `perf-notes.md` (tasks T042/T043) (missing)
