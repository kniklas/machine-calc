...
| Radio field keyboard contract | **Superseded, Phase 9 (same reversal as the row above — this row previously restated the now-superseded `RadioList`/Tab design directly beneath it, missed by an earlier correction pass and caught by a round-3 code-review pass on PR #96)**: Up/Down (`j`/`k`) always move between fields unconditionally, regardless of type — there is no Tab/Shift-Tab binding at all. Left/Right/Space cycle whichever field is currently selected: on a radio field, cycles its value with wraparound and commits immediately (no buffering); on a numeric field, nudges its buffer by a step (FR-017, unchanged) without committing until the user navigates away from it. |
