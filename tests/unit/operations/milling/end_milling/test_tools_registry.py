"""Unit tests for the EndMillTool registry (T020).
...
from machine_calc.operations.milling.end_milling.tools import (
...
_EXPECTED_BUNDLED_TOOLS = {"HSS": 1.0, "Cobalt": 1.15, "Carbide": 2.5, "Coated Carbide": 3.0}

_TABLE = "end_mill_tools"
...
def test_user_config_can_append_a_new_tool(tmp_path):
    ...
    names = list_end_mill_tools(path)

    assert names[: len(_EXPECTED_BUNDLED_TOOLS)] == list(_EXPECTED_BUNDLED_TOOLS)
    assert "Diamond" in names
    assert math.isclose(get_end_mill_tool("Diamond", path).cutting_speed_factor, 6.0, rel_tol=1e-9)
...
def test_milling_tools_need_no_feed_factor(tmp_path):
    ...
    tool = get_end_mill_tool("Fine", path)
