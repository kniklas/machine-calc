"""Validation-matrix tests for ``calculate_face_milling()`` (T032).
...
def _calc(**overrides):
    ...
    return calculate_face_milling(**kwargs)
...
def test_invalid_diameter_is_reported(diameter):
    _assert_error(_calc(diameter=diameter, **{"width_of_cut": 0.5}), "INVALID_DIAMETER")
...
@pytest.mark.parametrize("value", [0, -1, float("nan")])
def test_invalid_width_of_cut_is_reported(value):
    _assert_error(_calc(**{"width_of_cut": value}), "INVALID_DEPTH_OF_CUT")


def test_width_of_cut_exceeding_the_diameter_is_reported():
    """FR-009: geometrically impossible engagement is rejected, not clamped."""

    result = _calc(diameter=40.0, **{"width_of_cut": 41.0})

    _assert_error(result, "INVALID_ENGAGEMENT")
    assert "Width of cut" in result.error.message


def test_width_of_cut_equal_to_the_diameter_is_accepted():
    # Kept at/below ``max_depth_of_cut_mm`` so this exercises the FR-009
    # diameter relation rather than the separate configurable bound.
    result = _calc(diameter=40.0, **{"width_of_cut": 40.0})
