"""Unit tests for calculate_face_milling_metrics_at_rpm() (T010).

The wrapper delegates to the shared at-RPM core, so these tests pin the
mapping it is responsible for (width of cut -> radial engagement) and
confirm it produces the expected values for a caller-supplied spindle
...
def _call(**overrides):
    kwargs = {
        ...
        "spindle_speed_rpm": 800.0,
    }
    kwargs.update(overrides)
    return calculate_face_milling_metrics_at_rpm(**kwargs)
...
def test_spindle_speed_echoes_the_supplied_value_exactly():
    metrics = _call(spindle_speed_rpm=2718.0)

    assert metrics.spindle_speed_rpm == 2718.0
