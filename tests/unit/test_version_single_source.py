...
declares ``dynamic = ["version"]`` and reads ``machine_calc.__version__``, so
...
_PYPROJECT = Path(machine_calc.__file__).resolve().parents[2] / "pyproject.toml"


def test_runtime_version_matches_the_installed_distribution():
    assert machine_calc.__version__ == metadata.version("machine-calc")


def test_version_is_a_valid_release_identifier():
    assert re.fullmatch(r"\d+\.\d+\.\d+", machine_calc.__version__)
...
def test_pyproject_does_not_redeclare_a_static_version():
    ...
    assert not re.search(r"^version\s*=", project_table, re.MULTILINE), (
        ...
        "single source of truth is machine_calc.__version__ (Constitution IV)"
    )
...
def test_milling_release_is_at_least_the_minor_that_introduced_it():
    ...
    major, minor, _patch = (int(part) for part in machine_calc.__version__.split("."))

    assert (major, minor) >= (0, 3)
    assert hasattr(machine_calc, "calculate_end_milling")
    assert hasattr(machine_calc, "calculate_face_milling")
