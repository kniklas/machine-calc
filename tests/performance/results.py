...
def build_suite_run_summary(reports: list[PerformanceReport]) -> dict[str, Any]:
    ...
    # (which describe *enforcement* — i.e. best-effort mode on platforms
    # like macOS/Windows, which is allowed to still "pass" per FR-009/
    # FR-010): this instead reports whether every case's memory *reading*
    # itself was usable at all (issue #23). An invalid reading always means
    # `overall_pass=False` too, but the CI summary needs this as a separate
    # signal so it reports a real "fail" for an invalid measurement instead
    # of being masked by the (also-true, but weaker) "⚠️ degraded" case.
