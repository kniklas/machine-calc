"""End-milling calculations (specs/009-milling-calculations US2).

Public entry point :func:`calculate_end_milling`, plus the end-mill tool
...
from .formulas import EndMillingMetrics, calculate_end_milling_metrics
from .tools import EndMillTool, get_end_mill_tool, list_end_mill_tools

_ENGAGEMENT_LABEL_KEY = "cli.label.radial_depth_of_cut"
...
def _compute(
    ...
    return calculate_end_milling_metrics(
        diameter_mm=geometry_mm["diameter_mm"],
        axial_depth_of_cut_mm=geometry_mm["axial_depth_of_cut_mm"],
        radial_depth_of_cut_mm=geometry_mm["radial_engagement_mm"],
...
def calculate_end_milling(
    ...
    Args:
        diameter: End-mill diameter (D). Must be > 0 and within the
        ...
        radial_depth_of_cut: Radial depth of cut (ae). Must be > 0 and must
            not exceed ``diameter`` (FR-009).
        feed_per_tooth: Feed per tooth / chip load (fz). Must be > 0.
        number_of_teeth: Number of flutes (zn). Must be a positive whole
            number.
        length_of_cut: Distance the cutter travels through the workpiece,
        ...
        tool: Registered end-mill tool name (see
            :func:`list_end_mill_tools`).
    ...
    return calculate_milling(
        ...
        radial_engagement=radial_depth_of_cut,
        ...
        resolve_tool=get_end_mill_tool,
        compute=_compute,
